...
static void Jmetertestfile(int api,char* bot, char* sendbuffer, char* response, char* data, size_t baselen, FILE* sourcefile, FILE* out, int& passcnt, int& failcnt)
{
	...
	*second = 0;
	while (1) // each line of regression file
	{
		...
		*second = 0;
		Myfgets(second, 100000 - 100, sourceFile);
		if (*second == '"') // use it up
		{
			strcat(ptr, second);
			*second = 0;
...
static void ReadNextJmeter(char* name, uint64 value)
{
	...
	Myfgets(data, 100000 - 100, sourceFile); // skip header
	...
	*bot = 0;
	...
	Jmetertestfile(api,bot, sendbuffer, response, data, baselen, sourceFile, out, pass, fail);
	bool quit = false;
	if (quit) myexit(0);
	FreeBuffer();
	FreeBuffer();
