...
void AddError(char* buffer)
{
	...
	*seen = 0;
	...
	{
		sprintf(message, "%s ended %s line %u col %u ", buffer, seen, currentFileLine, currentLineColumn);
		strcat(message, "\r\n");
	}
	...
	while (ALWAYS) //   read as many tokens as needed to complete the definition
	{
		...
		switch(*word) // ordinary tokens, not the composite comparison blob
		{
			...
			case '{':	//   list of optional choices begins
				...
				if (nestKind[nestIndex - 1] == '<' && !memorizeSeen && !strstr(start,"^matches"))
			...
			case SYSVAR_PREFIX: //   system data
				...
				if (!word[1]); //   simple %
				else if (!FindWord(word) && stricmp(word,"%trace_on") && stricmp(word,"%trace_off"))
				{
					patternEnder = data;
...
#ifdef WIN32
	sprintf(botid, (char*)"%I64u", myBot);
#else
	sprintf(botid, (char*)"%llu", myBot);
#endif
