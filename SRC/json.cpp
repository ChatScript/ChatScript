...
static void  DoJSONAssign(WORDP leftside, WORDP keyname, char* value,bool bootfact,bool stripQuotes,char* fullpath)
{
	...
	unsigned int flags = (leftside->word[1] == 'a') ? JSON_ARRAY_FACT : JSON_OBJECT_FACT;
		...
		if (followOnSeparator) // goto next piece
		{
			*followOnSeparator = c; // restore normal values
			...
			{ // NOTE we cannot create an array index out of order. We can create the next index in line however.
				...
				if (c == '.') InternalCall("^JSONCreateCode", JSONCreateCode, arg1, (char*)"object", NULL, loc); // followon is an object, not an array
				else if (!arrayfact) InternalCall("^JSONCreateCode", JSONCreateCode, arg1, (char*)"array", NULL, loc); // we have no array facts yet
				else
				{
